...
By default, it uses 'linear'. But you can use your custom easing function from "this awesome easing library":http://gsgd.co.uk/sandbox/jquery/easing/.

h3. onLoadCallback

Optional callback fired after it initially loads the content( only when `iframe` is set `false`. )
